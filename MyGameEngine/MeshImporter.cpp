...
std::vector<MeshImporter::Ptr> MeshImporter::loadFromFile(const std::string& path) {
    ...
    //load meshes
    vector<MeshImporter::Ptr> mesh_ptrs;
    for (const auto& mesh_ptr : scene.meshes()) {
        ...
        auto mesh_sptr = make_shared<MeshImporter>(Formats::F_V3T2, vertex_data.data(), vertex_data.size(), index_data.data(), index_data.size());
...
MeshImporter::MeshImporter(Formats format, const void* vertex_data, unsigned int numVerts, const unsigned int* index_data, unsigned int numIndexs) :
    _format(format),
    _numVerts(numVerts),
...
MeshImporter::MeshImporter(MeshImporter&& b) noexcept :
    _format(b._format),
