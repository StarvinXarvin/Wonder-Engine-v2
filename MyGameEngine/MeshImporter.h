...
class MeshImporter : public Graphic
{
public:
...
public:
	using Ptr = std::shared_ptr<MeshImporter>;

	static std::vector<Ptr> loadFromFile(const std::string& path);

	Texture2D::Ptr texture;

	MeshImporter(Formats format, const void* vertex_data, unsigned int numVerts, const unsigned int* indexs_data = nullptr, unsigned int numIndexs = 0);
	MeshImporter(MeshImporter&& b) noexcept;
	void draw();
	~MeshImporter();

private:
	MeshImporter(const MeshImporter& cpy) = delete;
